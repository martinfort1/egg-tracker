...
export class SalesController {
  ...
  @Post()
  @UseGuards(JwtGuard)
  create(@Body() dto: CreateSaleDto, @GetUser() user: { id: string }) {
  ...
  @Get('/summary')
  @UseGuards(JwtGuard)
  getSummary(@GetUser('id') userId: string, @Query('period') period?: string) {
    return this.salesService.getSummary(userId, period);
  }

  @Get('/monthly')
  @UseGuards(JwtGuard)
  getMonthly(@GetUser('id') userId: string, @Query('period') period?: string) {
